...
if ( strpos($the_path_for_the_transient, 'new-business') === 0 ) {
...
// Sets up this page's cache.
$content_transient = get_transient( md5( $_SERVER["REQUEST_URI"] ) . '_content_' . $cached_time->timestamp );
$sendo_transient = get_transient( md5( $_SERVER["REQUEST_URI"] ) . '_sendo_' . $cached_time->timestamp );
...
if ( $content_transient && $sendo_transient && ! is_user_logged_in( ) ) {
	...
	if (get_field('seo_page_keywords', 'option') || get_field('seo_page_keywords')) {
		...
		if (is_array($keywords_to_get) && count($keywords_to_get) > 0) {
...
<script type="text/javascript">
	...
	console.log('<?php echo ( get_field( 'enable_caching', 'option' ) ) ? 'Caching is enabled' : 'Caching is disabled'; ?><?php echo ( is_user_logged_in() ) ? '. While you are logged in, visiting a page clears that page from the cache.' : '.'; ?>');
